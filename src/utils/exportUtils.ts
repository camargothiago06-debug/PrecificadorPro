...
export function exportProductsToCSV(products: ProductItem[]) {
  const headers = [
    ...
    'Volume Mensal (un)',
    'Volume Total Mensal (kg)',
    'Capacidade Fábrica (kg/mês)',
    'Custos Diretos (R$/un)',
    'Custos Diretos por kg (R$/kg)',
    'Insumos/Matéria-Prima (R$)',
    'Crédito Tributário Insumos (R$)',
    'Embalagem (R$)',
    'Mão de Obra Direta (R$)',
    'GGF - Custos Indiretos (R$/un)',
    'GGF Rateado por kg (R$/kg)',
    'Custo Total de Produção (R$/un)',
    'Custo de Produção por kg (R$/kg)',
    'Despesas Fixas Rateadas (R$/un)',
    'Despesas Fixas por kg (R$/kg)',
    'Custo Total Unitário (R$/un)',
    'Custo Total por kg (R$/kg)',
    'Impostos Saída (%)',
    'Impostos Saída (R$)',
    'Comissões e Taxas Variáveis (%)',
    'Despesas Variáveis Unit (R$)',
    'Margem de Lucro Desejada (%)',
    'Preço de Venda Sugerido (R$/un)',
    'Preço de Venda Praticado (R$/un)',
    'Preço de Venda por kg (R$/kg)',
    'Margem de Contribuição (R$/un)',
    'Margem de Contribuição (%)',
    'LAIR - Lucro Antes IRPJ/CSLL (R$)',
    'Provisão IRPJ & CSLL 34% (R$)',
    'Lucro Líquido Unitário (R$/un)',
    'Lucro Líquido por kg (R$/kg)',
  ...
  const rows = products.map((product) => {
    ...
    const totalMonthlyKg = (product.targetSalesVolume || 0) * (calc.netWeightKg || 1);
    ...
    return [
      ...
      calc.netWeightKg.toFixed(3),
      product.targetSalesVolume,
      totalMonthlyKg.toFixed(2),
      (product.factoryMonthlyKgCapacity || 0).toFixed(0),
      calc.totalDirectCosts.toFixed(2),
      calc.directCostsPerKg.toFixed(2),
      ...
      calc.ggfPerKg.toFixed(2),
      calc.totalProductionCostUnit.toFixed(2),
      calc.productionCostPerKg.toFixed(2),
      calc.totalFixedExpensesUnit.toFixed(2),
      calc.fixedExpensePerKg.toFixed(2),
      calc.totalUnitCost.toFixed(2),
      calc.totalCostPerKg.toFixed(2),
      ...
      calc.effectiveSalePricePerKg.toFixed(2),
